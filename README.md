...
`FocusSessionService` is a **started-only** `ForegroundService` (`onBind` returns `null`) whose sole responsibilities are (1) keeping the process alive via `startForeground`, (2) routing `onStartCommand` actions (`ACTION_START`, `ACTION_PAUSE`, `ACTION_RESUME`, `ACTION_STOP`) to the injected `FocusSessionController`, and (3) updating the persistent notification by observing `controller.state` reactively. Session state is owned by `FocusSessionRunner` (a `@Singleton` Hilt binding), so `HomeViewModel` injects `FocusSessionController` directly — no `ServiceConnection` or `IBinder` is needed. User actions in the UI emit typed `HomeEffect` values (`StartSessionService`, etc.) which the Composable boundary translates into `context.startService(intent)` calls. The service returns `START_NOT_STICKY` and handles a null restart intent by calling `stopSelf()` immediately.
